class Solution {
    ...
    checkInclusion(s1, s2) {
        const s1Occurances = this.occurances(s1);
        const windowSize = s1Occurances.reduce((sum, count) => sum + count, 0);

        for (let i = 0; i < s2.length - windowSize + 1; i++) {
            const substring = s2.substring(i, i+ windowSize);
            if (s1Occurances.toString() === this.occurances(substring).toString()) {
                return true;
            }
        }
